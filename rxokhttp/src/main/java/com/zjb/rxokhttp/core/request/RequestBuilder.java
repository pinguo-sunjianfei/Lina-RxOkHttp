...
public class RequestBuilder<T> {
    ...
    private String mRequestMethod = RxOkHttpRequest.Method.POST;
    ...
    public RequestBuilder<T> requestMethod(String method) {
    ...
    public HttpGsonRequest<T> build() {
        ...
        if (mParamsType == TYPE_NEED_BASE) {
            //1.1.添加基本参数
            RxOkHttp.sParametersGenerator.addBaseParameters(mHttpParams);
            //1.2.生成签名字段
            mHttpParams = RxOkHttp.sParametersGenerator.generateParameter(mUrl, mHttpParams);
        }
        //2 得到请求实体
        if (mIsMultipart) {
